// package
package net.sansa_stack.RDFQuery
...
class QuerySystem(
    ...
    def fetchQueries: ArrayBuffer[ArrayBuffer[String]] = {
        ...
        val fileScanner = new Scanner(new FileInputStream(new File(queryInputPath)))
        ...
        for (i <- 0 to query.length if !isEnd) {
            ...
            if (line.toUpperCase.startsWith("SELECT")) {
                ...
                if (selectLine.nonEmpty) {
                    consoleLog.error(s"Multiple SELECT lines detected: $line")
                ...
                if (line.toUpperCase.startsWith("WHERE")) {
                    ...
                    while (!line.endsWith(this.symbol("curly-bracket-right"))) {
                        ...
                        if (line.toUpperCase.startsWith("WHERE")) {
                            consoleLog.error(s"Multiple WHERE lines detected: $line")
                        ...
                        if (!line.toUpperCase.contains("FILTER")) {
                            ...
                            if (filterLine.nonEmpty) {
                                consoleLog.error(s"Only one Filter Operator is allowed per query: $line")
                    ...
                    for(k <- j+1 until query.size) {
                        ...
                        if (line.toUpperCase.startsWith("LIMIT")) {
                            ...
                            if (_queriesLimit.get(qID).isDefined) {
                                consoleLog.error(s"Only one LIMIT Operator is allowed per query: $line")
                            ...
                            // store LIMIT value
                            if (Try(queryLimit.toInt).isSuccess)
                                _queriesLimit = Map(qID -> queryLimit.toInt)
                            else {
                                consoleLog.error(s"Limit must be an Integer value! Supplied: $queryLimit")
                                throw new IllegalStateException(s"Limit must be an Integer value! Supplied: $queryLimit")
                            }
                        }
                    ...
                    consoleLog.error("WHERE Clause not found!")
    ...
    def validateVariables(variables: ArrayBuffer[String], whereVariables: ArrayBuffer[String], qID: Int, validateType: String = "SELECT"): Unit = {
        variables.foreach(variable => {
            if (!whereVariables.contains(variable)) {
                consoleLog.error(s"Query No. $qID: $validateType variables must be in WHERE clause: $variable")
    ...
    def queryEngine(qID: Int): Unit = {
        if (_unionOp(qID)("first")) {
            if (_unionOp(qID)("first").equals(_unionOp(qID)("last"))) consoleLog.info(s"Query No: ${qID + 1}")
            else consoleLog.info(s"Query No: ${qID + 1} - UNION")
        }

        // validate number of WHERE clause triples
        if (_numOfWhereClauseTriples(qID).equals(1)) {
            consoleLog.info("No. of WHERE clause Triples: 1")

            // process first triple
            this.runFirstTriple(qID)
        } else {
            consoleLog.info(s"No. of WHERE clause Triples: ${_numOfWhereClauseTriples(qID)}")
                ...
                if (lineSplit.length.equals(1) || lineSplit.length.equals(3)) {
                    ...
                    if (lineSplit.length.equals(3)) {
                        ...
                        if (!content.startsWith(this.symbol("question-mark"))) {
                            consoleLog.error(s"FILTER - Wrong Variable: $filterLine")
                            ...
                            if (lineSplit(1).nonEmpty) {
                                ...
                                consoleLog.error(s"FILTER - Empty Operator: $filterLine")
                                throw new IllegalStateException(s"FILTER - Empty Operator: $filterLine")
                            }
                        }
                        ...
                        if (lineSplit(0).nonEmpty &&
                            ...
                            consoleLog.error(s"FILTER - Wrong Function: $filterLine")
                            throw new IllegalStateException(s"FILTER - Wrong Function: $filterLine")
                        }
                    }
                } else {
                    consoleLog.error(s"FILTER - Wrong Statement: $filterLine")
                    throw new IllegalStateException(s"FILTER - Wrong Statement: $filterLine")
                }
            }
    ...
    def fetchFilterFunctionData(fName: String, filterFunction: String, processLine: String): ArrayBuffer[String] = {
        val data: ArrayBuffer[String] = ArrayBuffer()

        // split
        val splitData = filterFunction.split(fName)
        var locationPoint = splitData(1).indexOf(this.symbol("round-bracket-right"))
    ...
    def overallQueriesTime(): Unit = {
        ...
        if (milliseconds >= 1000) {
            consoleLog.info(s"--> Overall Process Time: ${milliseconds}ms (${seconds}secs approx.)")
        } else {
            consoleLog.info(s"--> Overall Process Time: ${milliseconds}ms")
        }
    }
}
